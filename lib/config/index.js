...
const readCLArgs = () => {
    ...
    if (program.manifestDest) {
        config.options = config.options || {};
        config.options.manifestDest = program.manifestDest;
    }
...
const config = () => {
    ...
    // config priority: 1) command line 2) buster.json 3) package.json
    const configuration = clConfig && { ...defaultConfig, ...clConfig } ||
        busterConfig && { ...defaultConfig, ...busterConfig } ||
        pjConfig && { ...defaultConfig, ...pjConfig };
