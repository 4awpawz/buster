...
const saveManifestToFile = (manifest) => {
    const writeableManifest = {
        "manifest": manifest
    };

    fs.writeFileSync(manifestFileName, JSON.stringify(writeableManifest));
};

const backupFiles = (manifest) => {
    const filt = manifest.filter(od => od.operation === 2);
    var count = 0;
...
const hashFiles = (manifest) => {
    const filt = manifest.filter(od => od.operation === 1 || od.operation === 3);
    var count = 0;
